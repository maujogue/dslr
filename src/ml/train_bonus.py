...
def calculate_average_loss(X, Y_dict, weights):
    return sum(
        loss(X, Y_dict[house], weights[house]) for house in HOUSES
    ) / len(HOUSES)
...
def train_all_optimizers(X, Y_dict, iterations, lr, optimizers):
    ...
    for name, train_func, kwargs in optimizers:
        ...
        weights, training_time = train_optimizer(
            X, Y_dict, iterations, lr, train_func, kwargs
        )
...
def display_performance_comparison(results):
    ...
    for name, result in results.items():
        ...
        print(
            f"{name:<12} Final Loss: {result['loss']:.6f}, "
...
def save_all_weights_bonus(weights_batch, weights_sgd, weights_mini):
    ...
    save_weights(
        weights_mini, f"{weights_dir}/weights_mini_batch.txt", "mini-batch"
    )

    print(f"\n{GREEN}All weights saved in {weights_dir}/:{RESET}")
    print(
        f"{GREEN}You can now use --bonus with logreg_predict.py "
...
def train_all_bonus(X, Y_dict, iterations, lr):
    ...
    print("Comparing Batch GD, SGD, and Mini-batch GD\n")
